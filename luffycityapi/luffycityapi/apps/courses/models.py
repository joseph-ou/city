...
class CourseDirection(BaseModel):
    name = models.CharField(verbose_name='方向名称',max_length=255,unique=True)
    remark=CKEditor5Field(default='',blank=True,null=True,verbose_name='方向描述')#blank控制表单验证的行为，表示该字段在Django的表单（如ModelForm）中是可选 null控制数据库存储的行为，表示该字段在数据库中可以存储NULL值
    recomment_home_pot=models.BooleanField(default=False,verbose_name='是否推荐到首页新课栏目')
...
class CourseCategory(BaseModel):
    ...
    remark = CKEditor5Field(default="", blank=True, null=True, verbose_name="分类描述")
...
class Course(BaseModel):
    ...
    },upload_to="course/cover", max_length=255, verbose_name="封面图片", blank=True, null=True)
    ...
    description = CKEditor5Field(null=True, blank=True, verbose_name="详情介绍")
...
class Teacher(BaseModel):
    ...
    brief = CKEditor5Field(max_length=1024, verbose_name="讲师描述")
...
class CourseChapter(BaseModel):
    ...
    summary = CKEditor5Field(blank=True, null=True, verbose_name="章节介绍")
    pub_date = models.DateField(auto_now_add=True, verbose_name="发布日期")
