...
#inlines用于在父模型的编辑页面里，直接管理子模型的数据。 当用户编辑一个学习方向时，请把与这个方向关联的所有课程分类数据也显示在同一个页面里，让管理员可以同时进行增删改查
...
class CourseDirectionModelAdmin(admin.ModelAdmin):
    """学习方向的模型管理器"""
    list_display = ["id", "name", "recomment_home_pot", "recomment_home_top"]
    # 默认排序字段
    ordering = ["id"]
    # 字段过滤
    list_filter = ["recomment_home_pot", "recomment_home_top"]
    # 搜索字段
    search_fields = ["name"]
    # 内嵌外键数据
